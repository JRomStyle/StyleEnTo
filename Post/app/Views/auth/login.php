<div class="max-w-md mx-auto bg-white rounded-3xl p-10 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] border border-slate-100">
    <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-black text-slate-900">Bienvenido</h1>
        <p class="text-slate-500 mt-2">Ingresa tus datos para continuar</p>
    </div>
    ...
    <form method="POST" action="<?= htmlspecialchars($basePath . '/login') ?>" class="space-y-5">
        <input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf ?? '') ?>">
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 ml-1">Email</label>
    ...
    <div class="mt-10 pt-8 border-t border-slate-50 text-center space-y-3">
        <p class="text-sm text-slate-500">
            ¿No tienes cuenta? 
            <a class="text-[#ffbc0d] font-black hover:underline" href="<?= htmlspecialchars($basePath . '/register') ?>">REGÍSTRATE</a>
        </p>
        <p class="text-sm text-slate-500">
            ¿Olvidaste admin? 
            <a class="text-slate-400 font-medium hover:text-slate-600 underline" href="<?= htmlspecialchars($basePath . '/admin-reset') ?>">Restablecer</a>
        </p>
    </div>
</div>
