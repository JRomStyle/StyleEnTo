<div class="max-w-md mx-auto bg-white rounded-3xl p-10 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] border border-slate-100">
    <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-black text-slate-900">Únete</h1>
        <p class="text-slate-500 mt-2">Crea tu cuenta para empezar</p>
    </div>
    ...
    <form method="POST" action="<?= htmlspecialchars($basePath . '/register') ?>" class="space-y-5">
        <input type="hidden" name="_csrf" value="<?= htmlspecialchars($csrf ?? '') ?>">
        <div>
            <label class="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 ml-1">Nombre</label>
    ...
    <div class="mt-10 pt-8 border-t border-slate-50 text-center">
        <p class="text-sm text-slate-500">
            ¿Ya tienes cuenta? 
            <a class="text-[#ffbc0d] font-black hover:underline" href="<?= htmlspecialchars($basePath . '/login') ?>">INGRESA AQUÍ</a>
        </p>
    </div>
</div>
