...
$stmt = db()->prepare('SELECT * FROM productos WHERE id = :id AND activo = 1 LIMIT 1');
$stmt->execute([':id' => $id]);
$p = $stmt->fetch();
if (!$p) {
...
$tallas = array_values(array_filter(array_map('trim', explode(',', (string) $p['tallas_csv']))));
if (count($tallas) === 0) {
    $tallas = ['U'];
}
...
<head>
  ...
  <title><?= e($p['nombre']) ?> — style en to!</title>
...
<body class="bg-white text-black">
  ...
  <main class="mx-auto max-w-7xl px-5 py-10">
    <div class="grid lg:grid-cols-2 gap-10 items-start">
      <div class="rounded-[2.5rem] overflow-hidden border border-black/10 shadow-2xl" data-reveal>
        <img class="w-full h-[560px] object-cover" src="<?= e(image_src((string) $p['imagen_url'])) ?>" alt="<?= e($p['nombre']) ?>" />
      </div>
      <div data-reveal>
        <a href="<?= e(url('tienda.php')) ?>" class="text-sm text-white/60 link-underline">← Volver a la tienda</a>
        <h1 class="mt-4 text-4xl md:text-5xl font-extrabold tracking-tight leading-[1.03]"><?= e($p['nombre']) ?></h1>
        <div class="mt-3 flex items-center gap-3">
          <div class="text-2xl font-extrabold">COP <?= number_format((float) $p['precio'], 2, ',', '.') ?></div>
          <div class="text-sm text-white/60"><?= e($p['categoria']) ?> · <?= e($p['tipo']) ?></div>
        </div>
        <p class="mt-6 text-white/70 leading-relaxed"><?= e($p['descripcion']) ?></p>

        <div class="mt-8 grid gap-4 max-w-sm">
          <div>
            <label class="text-sm font-semibold">Talla</label>
            <select id="size" class="mt-2 text-black w-full rounded-2xl border border-white/10 px-4 py-3 outline-none focus:border-black/30">
          ...
          <button type="button"
            class="px-7 py-3 rounded-full btn-primary font-semibold"
            data-add-to-cart-detail
            data-id="<?= (int) $p['id'] ?>"
            data-name="<?= e($p['nombre']) ?>"
            data-price="<?= e((string) $p['precio']) ?>"
            data-image="<?= e(image_src((string) $p['imagen_url'])) ?>"
          >Agregar al carrito</button>
          <div class="text-xs text-white/60">Pago simulado en checkout. No se realiza cobro real.</div>
        </div>
      </div>
    </div>
  </main>
