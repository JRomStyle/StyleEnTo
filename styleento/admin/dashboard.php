  ...
  <main class="mx-auto max-w-7xl px-5 py-10">
    <div class="flex items-end justify-between gap-6">
      <div>
        ...
        <div class="mt-2 text-white/60">Hola, <?= e((string) ($user['nombre'] ?? '')) ?>.</div>
      </div>
    ...
    <section class="mt-10 grid md:grid-cols-3 gap-5">
      <div class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
        <div class="text-sm text-black">Productos</div>
        <div class="mt-2 text-3xl font-extrabold text-black"><?= (int) $counts['productos'] ?></div>
      </div>
      <div class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
        <div class="text-sm text-black">Pedidos</div>
        <div class="mt-2 text-3xl font-extrabold text-black"><?= (int) $counts['pedidos'] ?></div>
      </div>
      <div class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
        <div class="text-sm text-black">Ventas (simulado)</div>
        <div class="mt-2 text-3xl font-extrabold text-black">COP <?= number_format((float) $counts['ventas'], 2, ',', '.') ?></div>
      </div>
    </section>
    ...
    <section class="mt-10 border border-black/10 rounded-[2.5rem] p-6 bg-white" data-reveal>
      <div class="flex items-end justify-between gap-6">
        <div>
          <div class="text-2xl text-black font-extrabold tracking-tight">Pedidos recientes</div>
      ...
      <div class="mt-6 grid gap-4">
        <?php foreach ($orders as $o): ?>
          ...
          <div class="border border-black/10 rounded-2xl p-5">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 text-black">
              <div class="min-w-0">
              ...
              <div class="flex items-center gap-4 text-black">
                <div class="font-extrabold">COP <?= number_format((float) $o['total'], 2, ',', '.') ?></div>
                <form method="post" class="flex items-center gap-2">
                  ...
                  <button class="px-5 py-2 rounded-full border border-black/10 hover:border-black/20 transition font-semibold text-green-600" type="submit">Guardar</button>
                </form>
              </div>
            </div>
              ...
              <div class="mt-4 border-t border-black/10 pt-4 grid gap-2 text-sm">
                <?php foreach ($items as $it): ?>
                  <div class="flex items-center justify-between gap-4 text-black">
                    ...
                    <div class="font-semibold text-black"><?= number_format((float) $it['subtotal'], 2) ?></div>
                  </div>
                <?php endforeach; ?>
              </div>
            <?php endif; ?>
        ...
        <?php if (count($orders) === 0): ?>
          <div class="border border-black/10 rounded-2xl p-6 text-black text-sm">Aún no hay pedidos.</div>
        <?php endif; ?>
      </div>
    </section>
  </main>
