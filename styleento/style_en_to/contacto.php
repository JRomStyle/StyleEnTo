  ...
  <main class="mx-auto max-w-7xl px-5 py-10">
    <div class="grid lg:grid-cols-2 gap-10 items-start">
      <div data-reveal>
        ...
        <p class="mt-3 text-black/60 max-w-xl">¿Tienes una duda, pedido o colaboración? Escríbenos y te respondemos.</p>
      ...
      <section class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
        <div class="text-xl font-extrabold tracking-tight">Enviar mensaje</div>
        <?php if ($ok): ?>
          <div class="mt-6 border border-accent/40 bg-accent/10 rounded-2xl p-4 text-sm">Mensaje enviado. Te contactamos pronto.</div>
          <a href="<?= e(url('index.php')) ?>" class="mt-6 inline-flex px-7 py-3 rounded-full btn-primary font-semibold">Volver al inicio</a>
        <?php else: ?>
          <?php if ($error !== ''): ?>
          ...
          <form class="mt-6 grid gap-4" method="post" action="<?= e(url('contacto.php')) ?>">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input name="nombre" value="<?= e($user['nombre'] ?? '') ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
              </div>
              <div>
                <label class="text-sm font-semibold">Email</label>
                <input name="email" type="email" value="<?= e($user['email'] ?? '') ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
              </div>
            </div>
            <div>
