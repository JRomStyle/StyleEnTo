  ...
  <main class="mx-auto max-w-7xl px-5 py-10">
    ...
    <?php if ($ok && $pedidoId): ?>
      <div class="mt-10 border border-black/10 rounded-3xl p-8 bg-white" data-reveal>
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-black/10 text-xs font-semibold tracking-wide uppercase">
          Confirmado <span class="w-1.5 h-1.5 rounded-full bg-accent"></span> Pedido #<?= (int) $pedidoId ?>
        </div>
        <div class="mt-5 text-3xl font-extrabold tracking-tight">Listo. Tu pedido fue creado.</div>
        <div class="mt-2 text-black/70">Puedes ver pedidos en el panel admin si eres dueño.</div>
        <div class="mt-7 flex flex-wrap gap-3">
          <a href="<?= e(url('tienda.php')) ?>" class="inline-flex px-7 py-3 rounded-full btn-primary font-semibold">Volver a la tienda</a>
          <a href="<?= e(url('index.php')) ?>" class="inline-flex px-7 py-3 rounded-full border border-black/10 hover:border-black/20 transition font-semibold">Inicio</a>
        </div>
        <script>
    ...
    <?php else: ?>
      ...
      <div class="mt-10 grid lg:grid-cols-[1fr_420px] gap-8 items-start">
        <section class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
          <div class="text-xl font-extrabold tracking-tight">Tus datos</div>
          <form id="checkout-form" class="mt-6 grid gap-4" method="post" action="<?= e(url('checkout.php')) ?>">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input name="nombre" value="<?= e($user['nombre'] ?? '') ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="Tu nombre" required />
              </div>
              <div>
                <label class="text-sm font-semibold">Email</label>
                <input type="email" name="email" value="<?= e($user['email'] ?? '') ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="tu@email.com" required />
              </div>
            </div>

            <div>
              <label class="text-sm font-semibold">Dirección</label>
              <input name="direccion" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="Calle, número, piso" required />
            </div>

            <div class="grid sm:grid-cols-2 gap-4">
              <div>
                <label class="text-sm font-semibold">Ciudad</label>
                <input name="ciudad" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
              </div>
              <div>
                <label class="text-sm font-semibold">País</label>
                <input name="pais" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
              </div>
            </div>

            <div>
              <label class="text-sm font-semibold">Teléfono</label>
              <input name="telefono" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="+34 ..." required />
            </div>

            <button class="mt-2 px-7 py-3 rounded-full btn-primary font-semibold" type="submit">Pagar (simulado)</button>
          </form>
        </section>
        ...
        <aside class="border border-black/10 rounded-3xl p-6 bg-white" data-reveal>
          ...
          <div id="checkout-summary" class="mt-6"></div>
          <div class="mt-6 flex items-center justify-between">
            <div class="text-black/60">Total</div>
            <div id="checkout-total" class="text-2xl font-extrabold">€0.00</div>
          </div>
