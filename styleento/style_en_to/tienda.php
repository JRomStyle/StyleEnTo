    ...
    <div class="border-b border-black/10">
      <div class="mx-auto max-w-7xl px-5 h-9 flex items-center justify-between text-xs">
        <span class="inline-flex items-center gap-2">
          ...
          Members: envío gratis en pedidos +50,00 COP
        </span>
    ...
    <div class="flex flex-col gap-6">
      <div class="flex items-end justify-between gap-6">
        <div>
          ...
          <div class="mt-2 text-white/60"><?= count($productos) ?> resultados</div>
        </div>
        <div class="flex items-center gap-3">
          <button type="button" data-toggle-filters class="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-white/10 hover:border-white/20 transition font-semibold text-sm">
          ...
          <form method="get" action="<?= e(url('tienda.php')) ?>">
            ...
            <input type="hidden" name="ofertas" value="<?= $ofertas === 1 ? '1' : '' ?>" />
            <input type="hidden" name="nuevo" value="<?= $nuevo === 1 ? '1' : '' ?>" />
            <select name="sort" class="rounded-full border border-black/10 bg-black/50 px-5 py-2 text-sm outline-none focus:border-black/30" onchange="this.form.submit()">
          ...
          <form class="mt-6 space-y-4" method="get" action="<?= e(url('tienda.php')) ?>">
            ...
            <div>
              ...
              <select name="categoria" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30">
                ...
                <option value="Hombre" <?= $categoria === 'Hombre' ? 'selected' : '' ?>>Men/Hombre</option>
                <option value="Mujer" <?= $categoria === 'Mujer' ? 'selected' : '' ?>>Women/Mujer</option>
      ...
      <div class="flex gap-8 items-start">
        <aside class="hidden lg:block w-72 shrink-0" data-desktop-filters>
          <div class="border border-black/10 rounded-[2rem] p-5 bg-white">
            <div class="text-black font-extrabold tracking-tight">Filtros</div>
            <form class="mt-5 space-y-4" method="get" action="<?= e(url('tienda.php')) ?>">
              <input type="hidden" name="q" value="<?= e($q) ?>" />
              <input type="hidden" name="sort" value="<?= e($sort) ?>" />
              <div>
                <label class="text-black font-semibold">Categoría</label>
                <select name="categoria" class="mt-2 text-black w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30">
                  <option value="">Todas</option>
                  <option value="Hombre" <?= $categoria === 'Hombre' ? 'selected' : '' ?>>Men/Hombre</option>
                  <option value="Mujer" <?= $categoria === 'Mujer' ? 'selected' : '' ?>>Women/Mujer</option>
                  <option value="Unisex" <?= $categoria === 'Unisex' ? 'selected' : '' ?>>Unisex</option>
              ...
              <div>
                <label class="text-black font-semibold">Tipo</label>
                <select name="tipo" class="mt-2 text-black w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30">
                  <option value="">Todos</option>
                  <?php foreach ($tipos as $t): ?>
                    <?php $v = (string) ($t['tipo'] ?? ''); ?>
                    <option value="<?= e($v) ?>" <?= $tipo === $v ? 'selected' : '' ?>><?= e(ucfirst($v)) ?></option>
              ...
              <div>
                <label class="text-black font-semibold">Talla</label>
                <input name="talla" value="<?= e($talla) ?>" class="mt-2 text-black w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="Ej: M o 42" />
              </div>
              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="text-black font-semibold">Precio mín</label>
                  <input name="min" value="<?= $min !== null ? e((string) $min) : '' ?>" class="mt-2 text-black w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" inputmode="decimal" placeholder="0" />
                </div>
                <div>
                  <label class="text-black font-semibold">Precio máx</label>
                  <input name="max" value="<?= $max !== null ? e((string) $max) : '' ?>" class="mt-2 text-black w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" inputmode="decimal" placeholder="120" />
                </div>
              </div>
              <div class="flex items-center gap-3">
              ...
              <div class="pt-2 flex gap-3">
                <button class="px-6 py-3 rounded-full btn-primary font-semibold" type="submit">Aplicar</button>
                <a class="px-6 py-3 rounded-full border border-black/10 hover:border-black/20 transition font-semibold text-black" href="<?= e(url('tienda.php')) ?>">Reset</a>
              </div>
            </form>
          </div>
        </aside>
            ...
            <a href="<?= e(url('producto.php') . '?id=' . (int) $p['id']) ?>" class="group block" data-reveal>
              <div class="relative overflow-hidden rounded-[2rem] border border-black/10 bg-white">
                <div class="aspect-[4/5] overflow-hidden">
                  <img class="w-full h-full object-cover opacity-95 group-hover:scale-[1.02] transition duration-300" src="<?= e(image_src((string) $p['imagen_url'])) ?>" alt="<?= e($p['nombre']) ?>" />
                </div>
                <div class="absolute inset-x-0 top-0 p-4 flex items-center justify-between gap-3">
                  ...
                  <?php endif; ?>
                  <button type="button"
                    class="w-11 h-11 rounded-full border border-black/10 bg-black/70 hover:border-black/20 transition inline-flex items-center justify-center"
                    data-add-to-cart
                    data-id="<?= (int) $p['id'] ?>"
                ...
                <div class="p-5">
                  ...
                  <div class="mt-2 text-black font-semibold">COP <?= number_format((float) $p['precio'], 2, ',', '.') ?></div>
