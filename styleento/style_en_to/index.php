    ...
    <section class="mx-auto max-w-7xl px-5 pt-6">
      <div class="relative overflow-hidden rounded-[2.75rem] border border-black/10">
        <div class="absolute inset-0">
          <img class="w-full h-[440px] md:h-[520px] object-cover opacity-90" src="<?= e(url('assets/img/hero-city-mode.jpg')) ?>" alt="" />
        ...
        <div class="relative p-8 md:p-12 max-w-2xl" data-reveal>
          ...
          <p class="mt-4 text-white/80 text-lg max-w-xl">
            Siluetas limpias, energía de barrio y acentos neón. Hecho para asfalto, noche y música.
          </p>
    ...
    <section class="mx-auto max-w-7xl px-5 py-10">
      <div class="flex items-end justify-between gap-6">
        <div>
          ...
          <p class="mt-2 text-black/60">Elige tu mood: calzado, ropa o accesorios.</p>
        </div>
      ...
      <div class="mt-6 grid md:grid-cols-3 gap-5">
        <a href="<?= e(url('tienda.php') . '?tipo=tenis') ?>" class="group relative overflow-hidden rounded-[2rem] border border-black/10 bg-white">
          <img class="w-full h-64 object-cover opacity-85 group-hover:scale-[1.02] transition duration-300" src="<?= e(url('assets/img/cat-tenis.jpg')) ?>" alt="" />
        ...
        <a href="<?= e(url('tienda.php') . '?tipo=hoodies') ?>" class="group relative overflow-hidden rounded-[2rem] border border-black/10 bg-white">
          <img class="w-full h-64 object-cover opacity-85 group-hover:scale-[1.02] transition duration-300" src="<?= e(url('assets/img/cat-hoodies.jpg')) ?>" alt="" />
          <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/25 to-transparent"></div>
          <div class="absolute left-6 bottom-6">
        ...
        <a href="<?= e(url('tienda.php') . '?tipo=gorras') ?>" class="group relative overflow-hidden rounded-[2rem] border border-black/10 bg-white">
          <img class="w-full h-64 object-cover opacity-85 group-hover:scale-[1.02] transition duration-300" src="<?= e(url('assets/img/cat-gorras.jpg')) ?>" alt="" />
          <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-black/25 to-transparent"></div>
          <div class="absolute left-6 bottom-6">
    ...
    <section class="mx-auto max-w-7xl px-5 py-14">
      <div class="flex items-end justify-between gap-6">
        <div>
          ...
          <p class="mt-2 text-black/60">Selección rápida con piezas que siempre vuelan.</p>
        </div>
        <a href="<?= e(url('tienda.php')) ?>" class="hidden sm:inline-flex px-6 py-2 rounded-full border border-black/10 hover:border-black/20 transition font-semibold">Ver todo</a>
      </div>
        ...
        <?php foreach ($best as $p): ?>
          <a href="<?= e(url('producto.php') . '?id=' . (int) $p['id']) ?>" class="group border border-black/10 rounded-2xl overflow-hidden bg-white hover:shadow-xl transition" data-reveal>
            <div class="aspect-[4/5] overflow-hidden">
              <img class="w-full h-full object-cover group-hover:scale-[1.03] transition duration-300" src="<?= e(url('assets/img/' . ltrim((string) $p['imagen_url'], '/'))) ?>" alt="<?= e($p['nombre']) ?>" />
            </div>
            <div class="p-4">
              <div class="font-semibold truncate"><?= e($p['nombre']) ?></div>
              <div class="mt-1 text-black/70 font-medium">€<?= number_format((float) $p['precio'], 2) ?></div>
    ...
    <section class="mx-auto max-w-7xl px-5 py-14">
      <div class="flex items-end justify-between gap-6">
        <div>
          ...
          <p class="mt-2 text-black/60">Lo último en drops: limpio, fuerte y listo para la calle.</p>
        </div>
            ...
            <a href="<?= e(url('producto.php') . '?id=' . (int) $p['id']) ?>" class="block group">
              ...
              <div class="p-4 flex items-center justify-between gap-3">
                <div class="min-w-0">
                  ...
                  <button type="button"
                  class="shrink-0 w-11 h-11 rounded-full border border-black/10 hover:border-black/20 transition flex items-center justify-center"
                  data-add-to-cart
                  data-id="<?= (int) $p['id'] ?>"
  ...
  <footer class="border-t border-black/10">
    <div class="mx-auto max-w-7xl px-5 py-12 grid md:grid-cols-3 gap-10">
      <div>
        ...
        <div class="mt-3 text-black/60">Streetwear premium inspirado en performance.</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div class="mt-3 text-sm text-black/70">
          ...
          <div class="mt-1">+34 600 000 000</div>
          <div class="mt-1">Ciudad · ES</div>
        </div>
      </div>
    </div>
    <div class="border-t border-black/10 py-6 text-center text-xs text-black/60">© <?= date('Y') ?> style en to! — Todos los derechos reservados.</div>
  </footer>
