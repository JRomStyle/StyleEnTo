  ...
  <main class="mx-auto max-w-7xl px-5 py-10">
    <div class="grid lg:grid-cols-2 gap-10 items-start">
      <div class="rounded-[2.5rem] overflow-hidden border border-black/10 shadow-2xl" data-reveal>
        <img class="w-full h-[560px] object-cover" src="<?= e(url('assets/img/' . ltrim((string) $p['imagen_url'], '/'))) ?>" alt="<?= e($p['nombre']) ?>" />
      </div>
      <div data-reveal>
        <a href="<?= e(url('tienda.php')) ?>" class="text-sm text-black/60 link-underline">← Volver a la tienda</a>
        <h1 class="mt-4 text-4xl md:text-5xl font-extrabold tracking-tight leading-[1.03]"><?= e($p['nombre']) ?></h1>
        <div class="mt-3 flex items-center gap-3">
          <div class="text-2xl font-extrabold">€<?= number_format((float) $p['precio'], 2) ?></div>
          <div class="text-sm text-black/60"><?= e($p['categoria']) ?> · <?= e($p['tipo']) ?></div>
        </div>
        <p class="mt-6 text-black/70 leading-relaxed"><?= e($p['descripcion']) ?></p>

        <div class="mt-8 grid gap-4 max-w-sm">
          <div>
            <label class="text-sm font-semibold">Talla</label>
            <select id="size" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30">
          ...
          <div class="text-xs text-black/60">Pago simulado en checkout. No se realiza cobro real.</div>
        </div>
      </div>
    </div>
  </main>
