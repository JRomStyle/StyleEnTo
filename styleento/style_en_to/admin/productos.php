      ...
      <section class="lg:w-[460px] shrink-0">
        <div class="border border-black/10 rounded-[2.5rem] p-6 bg-white" data-reveal>
          <div class="flex items-end justify-between gap-4">
            <div>
              <div class="text-2xl font-extrabold tracking-tight"><?= $edit ? 'Editar producto' : 'Nuevo producto' ?></div>
              <div class="mt-1 text-black/60 text-sm">Imágenes por URL (puedes usar Unsplash o tus assets).</div>
            </div>
            <?php if ($edit): ?>
              <a href="<?= e(url('admin/productos.php')) ?>" class="px-5 py-2 rounded-full border border-black/10 hover:border-black/20 transition text-sm font-semibold">Nuevo</a>
            <?php endif; ?>
          </div>
          ...
          <form class="mt-6 grid gap-4" method="post" action="<?= e(url('admin/productos.php') . ($edit ? '?edit=' . (int) $edit['id'] : '')) ?>">
            ...
            <div>
              ...
              <input name="nombre" value="<?= e((string) ($edit['nombre'] ?? '')) ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
            </div>
            <div>
              <label class="text-sm font-semibold">Precio (€)</label>
              <input name="precio" value="<?= e((string) ($edit['precio'] ?? '')) ?>" inputmode="decimal" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required />
            </div>
            <div>
              <label class="text-sm font-semibold">Categoría</label>
              <?php $cat = (string) ($edit['categoria'] ?? 'Unisex'); ?>
              <select name="categoria" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required>
            ...
            <div>
              ...
              <input name="tipo" value="<?= e((string) ($edit['tipo'] ?? '')) ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="hoodies, camisas, tenis..." required />
            </div>
            <div>
              <label class="text-sm font-semibold">Tallas (CSV)</label>
              <input name="tallas_csv" value="<?= e((string) ($edit['tallas_csv'] ?? '')) ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="XS,S,M,L,XL o 39,40,41..." required />
            </div>
            <div>
              <label class="text-sm font-semibold">Imagen (archivo en assets/img)</label>
              <input name="imagen_url" value="<?= e((string) ($edit['imagen_url'] ?? '')) ?>" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" placeholder="hoodie-neon-pulse.jpg" required />
            </div>
            <div>
              <label class="text-sm font-semibold">Descripción</label>
              <textarea name="descripcion" rows="5" class="mt-2 w-full rounded-2xl border border-black/10 px-4 py-3 outline-none focus:border-black/30" required><?= e((string) ($edit['descripcion'] ?? '')) ?></textarea>
            </div>
      ...
      <section class="flex-1">
        <div class="flex items-end justify-between gap-6">
          <div>
            ...
            <div class="mt-2 text-black/60"><?= count($productos) ?> en catálogo</div>
          </div>
        </div>
            ...
            <table class="w-full text-sm">
              ...
              <tbody class="divide-y divide-black/10">
                <?php foreach ($productos as $p): ?>
                  <tr>
                    <td class="py-3 pr-4">
                      <div class="font-semibold"><?= e((string) $p['nombre']) ?></div>
                      <div class="text-xs text-black/60">#<?= (int) $p['id'] ?> · <?= e((string) $p['creado_en']) ?></div>
                    </td>
                    <td class="py-3 pr-4"><?= e((string) $p['categoria']) ?></td>
                    <td class="py-3 pr-4"><?= e((string) $p['tipo']) ?></td>
                    <td class="py-3 pr-4 font-semibold">€<?= number_format((float) $p['precio'], 2) ?></td>
                    <td class="py-3 pr-4">
                      <?php if ((int) $p['activo'] === 1): ?>
                        <span class="inline-flex px-3 py-1 rounded-full border border-black/10">Activo</span>
                      <?php else: ?>
                        <span class="inline-flex px-3 py-1 rounded-full border border-black/10 text-black/60">Inactivo</span>
                      <?php endif; ?>
                    </td>
                    <td class="py-3 pr-4">
                      <div class="flex flex-wrap gap-2">
                        <a class="px-4 py-2 rounded-full border border-black/10 hover:border-black/20 transition font-semibold" href="<?= e(url('admin/productos.php') . '?edit=' . (int) $p['id']) ?>">Editar</a>
